...
function renderPage(items) {
    if (items.length === 0) {
        return renderHeader() + emptyState('newspaper', 'No news articles yet', 'Publish your first news article.') + renderFooter();
    }
...
function renderTable(items) {
    return `
        <table class="w-full text-sm">
            ...
            <tbody>
                ${items.map((n, i) => `
                    <tr data-id="${n.id}" class="group border-b border-stone-100 hover:bg-stone-50 transition-colors
